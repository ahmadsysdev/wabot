module.exports = {
    ...
    async run(client, message, { queries, mentioned, isGroup }) {
        ...
        // Object to store the results of adding premium
        const results = { success: [], failed: [], already: [] };

        // Import the addPremium function
        const { addPremium } = require('../../lib/senderType');
        
        // Process each mentioned participant
        mentioned.forEach(participant => {
            // Check if participant is already a premium/pro user
            ...
            // Add premium for the participant
            if (addPremium(participant, query)) {
        ...
        if (results.failed[0]) {
            await client.sendMessage(message.from, { text: `Upgrade of ${results.failed.map(value => '@' + value.replace('@s.whatsapp.net', '')).join(', ')} to premium failed.`, withTag: true }, { quoted: message });
